...
def ping_network(network_range):
    ...
    try:
        ...
        host_count = min(254, network.num_addresses - 2)  # Subtract network and broadcast addresses
...
def print_devices(devices):
    ...
    for device in devices:
        ...
        print(f"{device['ip']:<16} {format_mac(device['mac']):<18} {interface:<10} {hostname}")
...
def scan_network(network_range=None, interface=None):
    ...
    if not devices and methods_tried < 3:
        ...
        if proc_devices:
            ...
            if network_range:
                network = ipaddress.IPv4Network(network_range)
                devices = [d for d in proc_devices if ipaddress.IPv4Address(d['ip']) in network]
            else:
    ...
    if methods_tried == 0:
        ...
        print("Please make sure you have permissions to access network information or necessary network tools are installed")
    ...
    if args.output and unique_devices:
        try:
            with open(args.output, 'w') as f:
                ...
                f.write(f"{'IP Address':<16} {'MAC Address':<18} {'Interface':<10} {'Hostname'}\n")
                f.write("-" * 75 + "\n")
                for device in unique_devices:
                    hostname = get_hostname(device['ip'])
                    interface = device.get('interface', 'Unknown')
                    f.write(f"{device['ip']:<16} {format_mac(device['mac']):<18} {interface:<10} {hostname}\n")
